...
const useForecast = () => {
  ...
  //const test = `http://api.openweathermap.org/geo/1.0/direct?q=${value.trim()}&limit=5&appid=${process.env.NEXT_PUBLIC_REACT_APP_API_KEY}`
    const getSearchOptions = (value: string) => {
      fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${value.trim()}&limit=5&appid=${process.env.NEXT_PUBLIC_REACT_APP_API_KEY}`, {method: "GET", referrerPolicy: "origin", mode: "no-cors"})
      .then((res) => res.json())
